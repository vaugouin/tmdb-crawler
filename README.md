...
- `T_WC_TMDB_MOVIE_RELEASE_DATE` - every row returned by `/movie/{id}/release_dates`, including country, language, certification, note, exact source timestamp, normalized UTC datetime, release type, and descriptors. It is deliberately independent from `T_WC_TMDB_MOVIE.DAT_RELEASE`, whose established Movie Details retrieval is unchanged. `LANG` is normalized before storage: TMDb sometimes returns the whole language object instead of its code (`{"folded_name"=>"Swedish", "iso_639_1"=>"sv", ...}`), which overflowed the column and used to abort the title's whole snapshot; the crawler now recovers the embedded code and stores NULL when no code is readable, silently, since the repair calls for no decision. The `LANG` health query in [doc/sql/monitoring.sql](doc/sql/monitoring.sql) is what surfaces a recurrence. A NULL `LANG` therefore means TMDb published no usable language for that release event, not a crawler miss.
...
- **51-53**: Apply incremental changes from the TMDb `/changes` API (movies, persons, series, including selective season/episode refresh). Each entity walks one day at a time from a stored cursor, rewound 2 days at the start of a run, until it catches up with today. **When a page of a day's feed fails after its retries, the cursor no longer advances** (fixed 2026-09-14): that day is replayed on the next run instead of being declared processed, since everything after the failing page used to be lost silently. Past 14 days of lag the window `/changes` serves is closed, so the cursor then advances anyway and says so, rather than stalling the catch-up on an unrecoverable day
